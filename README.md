...
An Antigravity & Agentic IDE Skill that unifies autonomous codebase fact-finding, dynamic decision tree traversal, interactive modal panel questioning (`ask_question` tool), continuous deep grilling, **active domain modeling (`CONTEXT.md` & ADR crystallization)**, and **2-question incremental in-place plan document synchronization with zero data loss**.
...
2. **The Assumption Trap:** Agents jumping into code prematurely based on unverified assumptions and vague terminology, causing massive rework and diff churn.
...
- **Domain Modeling & Ubiquitous Language (`domain-modeling`):** Actively checks and challenges terminology against `CONTEXT.md`. Sharpens fuzzy language and captures emerging entities and irreversible trade-offs (`docs/adr/`) directly during the interview.
...
- **⚡ 2-Question Incremental In-Place Sync Cadence:** Every 2 questions answered, the agent immediately persists settled decisions into the active plan document on disk (`docs/plan-*.md` or `implementation_plan.md`) and domain artifacts with **Zero Data Loss**, ensuring progress is never lost, and immediately resumes the grilling loop.
...
This skill is a unified synthesis and architectural evolution of three foundational disciplines:
...
* **`domain-modeling`:** Enforces Ubiquitous Language integrity, glossary alignment against `CONTEXT.md`, and inline ADR crystallization.
* **Delizade Directive:** Plan-only execution boundary that guarantees the agent never touches application source code (`src/`), combined with the 2-question in-place plan sync protocol with zero data loss.
...
## Structure
- [`SKILL.md`](SKILL.md): Authoritative operational rules, workflow phases, modal panel directives, domain modeling integration, 2-question sync cadence, and plan synthesis specifications.
